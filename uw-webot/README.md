...
UW-Webot 旨在解决传统 Web 自动化框架在资源利用率和并发性能方面的瓶颈。通过 Hybrid 混合模式，实现了：

- **浏览器进程级复用**：同一个 BrowserInstance 可承载多个 BrowserTab，共享浏览器进程，降低内存与启动开销
- **页签级隔离（一次性 Context）**：每个 BrowserTab 拥有独立的 BrowserContext + Page（独立的 Cookie、Storage、代理）。每次 `openBrowserTab` 新建，`close()` 后销毁——经实测 Playwright Context 池化复用不稳定，因此不复用，仅 Browser 进程本身由池持有共享
- **组内轮询负载均衡**：BrowserGroup 内按活跃 Tab 数选择最空闲的 BrowserInstance，按需创建至上限
- **企业级功能**：内置验证码识别、反检测（通过 `addInitScript` 在页面脚本前注入）、代理池等高级特性
...
- **本地 OCR**：占位实现，未集成真实引擎（直接返回识别失败，生产请用下列云服务）
- **2Captcha**：第三方验证码解决服务（API key 通过 POST body 传递）
- **Capsolver**：AI 驱动的验证码解决服务

### 3. 反检测功能

通过 `BrowserContext.addInitScript` 在每个页面任何站点脚本之前注入：
...
- 健康检查（基于 httpbin 同步探测，失败次数超限自动跳过）
- 按类型筛选代理
- 轮询取用，命中代理保留在池中循环复用

### 5. 会话管理

- 基于 FusionCache（可本地、可分布式）
- 会话过期由缓存 TTL 控制
- 会话级存储状态（storageState）可保存与恢复
...
1. **Browser 进程级复用**：同一 BrowserInstance 可承载多个 BrowserTab，共享浏览器进程与 Playwright 实例
2. **Tab 级别隔离（一次性 Context）**：每个 BrowserTab 拥有独立的 BrowserContext（Cookie、Storage、代理）。每次新建、`close()` 后销毁，**不复用 Context**
3. **轮询负载均衡**：BrowserGroup 内按活跃 Tab 数选最空闲的 BrowserInstance，按需创建至上限
4. **超时机制**：BrowserInstance 的单线程 executor 默认 60s 超时，超时会标记实例失效并异步重建
5. **健康检查**：后台线程定期剔除已失效（`isActive()==false`）的 BrowserInstance
...
> 字段以 `uw.webot.conf.WebotProperties` 及各 `*Config` 类为准。`captcha` / `proxy` / `stealth` 均为 `Map<String, XxxConfig>`，key 即业务方在 `SessionConfig` 的 `*ConfigKey` 字段中引用的名称。
    ...
    session:
      ...
      default-session:
        ...
        browser-config:
          ...
          # user-agent / locale / timezone / args / executable-path 可选

    # 验证码服务 (Map<String, CaptchaConfig>)
    captcha:
      default:
        service-type: capsolver      # ocr | twocaptcha | capsolver
        api-key: "your-api-key"
        max-retries: 3
        max-timeout: PT2M            # 轮询结果的最大等待时长

    # 反检测服务 (Map<String, StealthConfig>)
    stealth:
      default:
        ...
        # user-agents / screen-resolutions / timezones: 自定义列表（留空用内置）

    # 代理服务 (Map<String, ProxyConfig>)
    proxy:
      default:
        max-failures: 3
...
| `updateSession(WebotSession, Duration)` | 更新会话（续期 TTL） | 会话, TTL | void           |
...
| `getSessionManager()`                  | 获取会话管理器     | 无       | SessionService |
...
| `addInitScript(String)`                             | 注册初始化脚本（navigate 前）| 脚本      | void      |
...
| `close()`                                           | 销毁 Context/Page 并注销 | 无       | void      |
...
| 服务类型      | 适用场景                | 成本 | 说明 |
|-----------|---------------------|----|-----|
| OCR       | 本地图片验证码（需自行集成引擎）       | 低  | 内置为占位实现，未集成真实引擎，直接返回失败 |
| 2Captcha  | 复杂验证码、ReCaptcha     | 中  | 生产可用 |
| Capsolver | AI 验证码、ReCaptcha V3 | 中高 | 生产可用 |
