...
| **多方言支持**        | 内置 MySQL / Oracle / SQL Server / PostgreSQL 分页方言，自动识别驱动类型                          |
...
| **双入口设计**        | `DaoManager`（推荐，`ResponseData` 包装）+ `DaoFactory`（传统，抛异常风格）            |
...
> **推荐使用 `DaoManager`**。`DaoFactory` 为传统入口，仅供历史代码兼容与需要细粒度异常控制的场景，
> 新代码统一用 `DaoManager`。
...
| 查询无数据/更新0行 | 返回 `ResponseData.warnCode(DATA_NOT_FOUND_WARN)`，`getData()` 为 null | load 返回 `null`；更新返回 `0` |
| 生产环境    | 异常只返回错误码，不泄漏 SQL/参数                              | 抛异常含完整 SQL+参数              |
| 事务/批量    | 直接 `beginTransaction()` / `beginBatchUpdate()`（委派 DaoFactory） | 直接 `beginTransaction()`    |
| 适用场景    | 新项目、Spring MVC 服务层、Helper 静态工具类                 | 历史代码、细粒度异常控制               |
...
> **读库路由**：`load()` 本质为读操作，路由到读库（`read-pools`）；未配置读库时自动回退写库。
> 配置读写分离时注意从库复制延迟，"写后立即读自身"场景可能读到旧数据，必要时手动指定写库 connName。
...
设置隔离级别（仅显式调用后才应用到连接，未设置则保持驱动默认）：
...
> 事务与批量更新 API（`beginTransaction` / `beginBatchUpdate`）已由 `DaoManager` 直接委派，
> 可用 `dao.beginTransaction()` / `dao.beginBatchUpdate()` 调用，无需经 `getDaoFactory()` 中转。
...
> **重要**：批量更新**必须在事务中运行**。`beginBatchUpdate()` 后需先 `beginTransaction()`，否则
> `PreparedStatement` 处于 autoCommit 状态，`addBatch()` 的数据会被立即自动提交，`executeBatch()` 与
> `submit()` 无法统一控制事务边界，框架会抛出 `TransactionException`。

```java
DaoManager dao = DaoManager.getInstance();
TransactionManager tx = dao.beginTransaction();    // 必须先开启事务
...
```java
// 获取下一个序列 ID（为实体类生成，seqName = 类简单名 User）
...
| 持久化  | 全量持久化到 `sys_seq` 表         | DB 段申请 + Redis 段缓存（Lua 原子取号/续段） |
| 适用场景 | 中低并发                       | 高并发 ID 生成                         |

> **实现自动选择与降级**：框架在配置了 `uw.dao.redis` **且** Redis 句柄就绪时使用 `FusionSequenceFactory`，
> 否则使用 `DaoSequenceFactory`。即使配置了 Redis，若 Redis 初始化失败/不可用，会自动降级到 DB 实现，
> 保证全局发号不因 Redis 故障而中断。运行期 Redis 瞬时抖动由 Fusion 内部指数退避重试（最多50次）兜底。

> **`getCurrentId` 语义说明**：该方法返回"最近一次已发出的号"，**并非数据库真值**，且两种实现语义有别：
> - `DaoSequenceFactory`：返回本地号段缓存的游标值（非 DB `sys_seq.seq_id` 真值，集群各实例不同）。
> - `FusionSequenceFactory`：返回 Redis 段缓存的游标值（最近一次 `getSequenceId` 发出的号）。
>
> 未初始化时返回 `0`。该方法仅用于观测，**不要用于业务判断或作为下一个待发号**。如需 DB 真值请直接查询
> `sys_seq` 表。
...
CREATE TABLE sys_seq (
    seq_name      VARCHAR(200) NOT NULL PRIMARY KEY COMMENT '序列名称',
    seq_id        BIGINT       DEFAULT 0            COMMENT '当前序列值',
    seq_desc      VARCHAR(200)                       COMMENT '序列描述',
    increment_num INT          DEFAULT 1            COMMENT '每次预取步长',
    create_date   DATETIME(3)                        COMMENT '创建时间',
    last_update   DATETIME(3)                        COMMENT '修改时间'
);
```
...
CREATE TABLE `sys_seq` (
  ...
  `seq_id`        BIGINT       DEFAULT NULL COMMENT '当前序列值',
  `seq_desc`      VARCHAR(200) DEFAULT NULL COMMENT '序列描述',
  `increment_num` INT          DEFAULT NULL COMMENT '每次预取步长',
  `create_date`   DATETIME(3)  DEFAULT NULL COMMENT '创建时间',
  `last_update`   DATETIME(3)  DEFAULT NULL COMMENT '修改时间',
...
### Q1：DaoManager.load() 无数据时返回什么？如何判空？

`load` / `queryForObject` / `queryForValue` 等查询方法在**查不到数据（或结果为 null）时返回 `DATA_NOT_FOUND_WARN`**，
此时 `getData()` 为 null（`isSuccess()` 为 false）。更新/删除 0 行同样返回 `DATA_NOT_FOUND_WARN`。建议：
...
### Q7：Oracle / SQL Server / PostgreSQL 如何接入？
...
uw:
  dao:
    conn-pool:
      root:
        driver: oracle.jdbc.driver.OracleDriver      # 或 com.microsoft.sqlserver.jdbc.SQLServerDriver / org.postgresql.Driver
        url: jdbc:oracle:thin:@localhost:1521:ORCL   # 或 jdbc:sqlserver://... / jdbc:postgresql://localhost:5432/mydb
```

支持驱动：`mysql` / `oracle` / `sqlserver` / `postgresql`（按 `driver` 类名包含的关键字匹配，未匹配时回退 MySQL 方言）。
